...
public class RestRouteBuilder extends RouteBuilder {
   ...
   public void configure() throws Exception {
	   ...
	   .log("Body before the call : ${body}")
        .setHeader("ObjectId", xpath("person/@user", String.class))
        .log("Header 'ObjectId' is : ${header.ObjectId}" )
        .processRef("toLower")
        .to("file:src/data/out")
        /*
        .setHeader(CacheConstants.CACHE_OPERATION, constant(CacheConstants.CACHE_OPERATION_CHECK))
        .setHeader(CacheConstants.CACHE_KEY, header("ObjectId"))
        .delay(50)
        .to("cache://replyCache?memoryStoreEvictionPolicy=MemoryStoreEvictionPolicy.LRU")
        .log("After cache check: ${headers}")
        .choice().when(header(CacheConstants.CACHE_ELEMENT_WAS_FOUND).isNotNull())
	        .setHeader(CacheConstants.CACHE_OPERATION, constant(CacheConstants.CACHE_OPERATION_GET))
	        .setHeader(CacheConstants.CACHE_KEY, header("ObjectId"))
	        .to("cache://replyCache?memoryStoreEvictionPolicy=MemoryStoreEvictionPolicy.LRU")
	       .otherwise()
	       	.log("element not found in cache")
        .end()
        */
        .to("consumerProcessor")
        .transform(body());

	   // This is the route from File to EH cache
	   from("file:src/data/out?moveFailed=error")
	   .marshal().string("UTF-8")
	   .log("Body after the file polling is: ${body}")
		//.setExchangePattern(ExchangePattern.InOut)
		.setHeader(CacheConstants.CACHE_OPERATION, constant(CacheConstants.CACHE_OPERATION_ADD))
		.setHeader(CacheConstants.CACHE_KEY, xpath("person/@user", String.class))
		.to("cache://replyCache?memoryStoreEvictionPolicy=MemoryStoreEvictionPolicy.LRU")
